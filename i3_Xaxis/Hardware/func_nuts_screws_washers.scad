...
module screw_from_object(screw_object){
    ...
    union() {
        ...
        // create the screw body/threads
            translate([0, 0, -0.001]) {
                _cylinder(h=screw_height+0.001, r=screw_dia(screw_type)/2, $fn=hole_fn);
                ...
                if (screw_head_height(screw_type) > 0)
                    translate([0, 0, -head_height+screw_head_offset(screw_type)]) {
                        ...
                        translate([0, 0, -0.001])
                            _cylinder(h=head_height+0.002, d2=head_bottom_dia, d1=head_top_dia, $fn=head_fn);
                }
            }
        }
    }
    ...
    difference() {
        union() {
            ...
            translate([0, 0, head_drop]) {
                _cylinder(h=screw_height+0.002, r=screw_dia(screw_type)/2, length=slot_length, fn=hole_fn);
                // add in the head if it's a countersunk screw

                if (head_height>0)
                    _cylinder(h=head_height, r2=head_bottom_dia/2, r1=head_top_dia/2, length=slot_length, fn=head_fn);
    
            }
            // if there is a head drop, add it
            if (head_drop>0)
                _cylinder(h=head_drop+0.001, r=head_top_dia/2, length=slot_length, fn=head_fn);
        }
        // hole support for the head if the screw is upside down (allows bridging)
        render(convexity = 6)
            translate([0, 0, head_drop+head_height])
                if (hole_support) {
                    _cylinder(h=layer_height*2+0.002, r=head_bottom_dia/2+0.001, length=slot_length, fn=head_fn);
                }
    }
}
...
module rod_hole(d=0, h=0, allowance=-1, length=0, $fn=0, center=false, horizontal=false){
	...
	_cylinder(h=h, r=dia/2, length=length, $fn=$fn, center=center);
}
...
module nut_obj(screw_object=-1, thickness=0){
	...
	if (screw_object != -1 && screw_obj_nut(screw_object) != -1) {
		...
		_cylinder(h=((thickness>0) ? thickness : nut_thickness(nut_type)), r=nut_outer_dia(nut_type)/2, $fn=6);
	} else {
...
module nut_obj_hole(screw_object=-1, thickness=0, nut_slot=0){
	if (screw_object != -1 && screw_obj_nut(screw_object) != -1) {
		...
		render() union() {
			hull() {
		 		rotate([0,0,180/6])
		 		_cylinder(h=nut_thickness, r=nut_outer_dia(nut_type)/2, $fn=6);
			...
			if (!horizontal) for(i = [start:stop])
				rotate([0,0,60*i]) translate([nut_outer_dia(nut_type)/2-corner_cutout_dia/2,0,0]) rotate([0,0, 180/(corner_cutout_fn/2)]) _cylinder(r=corner_cutout_dia/2, h=nut_thickness, $fn=corner_cutout_fn);
		}
	} else {
...
module washer(type=washer_M3, center=false){
	...
	render(convexity = 4) translate([0,0,(center) ? -washer_thickness(type)/2 : 0]) 
	    difference() {
	        _cylinder(h=washer_thickness(type), d=washer_outer_dia(type));
	        _cylinder(h=washer_thickness(type)+1, d=washer_dia(type));
	    }
}
...
module washer_hole(type=washer_M3, fn=0, horizontal=false){
	...
	_cylinder(h=washer_thickness(washer_h)+0.001, r=washer_outer_dia(washer_h)/2, fn=fn);
}
...
module screw_trap(l=20, screw=screw_M3_socket_head, nut=nut_M3, add_inner_support=0.5, outer_radius_add=2, fn=8, horizontal=false){
	...
	intersection() {
		...
		union() {
			translate([0, 0, l/2])
				_cylinder(r2=inner_r, r1=inner_r + outer_radius_add, h=outer_radius_add, $fn=fn);
			
			translate([0, 0, -l/2-outer_radius_add])
				_cylinder(r1=inner_r, r2=inner_r + outer_radius_add, h=outer_radius_add, $fn=fn);
			
			translate([0, 0, 0])
				_cylinder(r=inner_r + outer_radius_add, h=l+0.002, $fn=$fn, center=true);
		}
